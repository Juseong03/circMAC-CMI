...
Table~\ref{tab:pretrain} evaluates different self-supervised pretraining
objectives for \model{}.
Note that experiments combining MLM+CPCL, MLM+BSJ-MLM, and the full
combination (MLM+CPCL+BSJ-MLM) are currently ongoing;
we report available results here.

\begin{table}[htbp]
\centering
\caption{Effect of pretraining strategies on sites prediction.
         \dag~Experiments in progress.}
\label{tab:pretrain}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lccc}
\toprule
\textbf{Pretraining} & \textbf{F1-macro} & \textbf{AUROC} & \textbf{AUPRC} \\
\midrule
None (from scratch)   & 0.7456\spm 0.0028 & 0.8948\spm 0.0046 & 0.5182\spm 0.0093 \\
MLM                   & 0.7585\spm 0.0021 & 0.8976\spm 0.0005 & 0.5326\spm 0.0049 \\
CPCL                  & \best{0.7611\spm 0.0027} & \best{0.9077\spm 0.0009} & \best{0.5473\spm 0.0060} \\
SSP                   & 0.7383\spm 0.0016 & 0.8930\spm 0.0023 & 0.4906\spm 0.0025 \\
MLM + CPCL            & ---\dag & ---\dag & ---\dag \\
MLM + BSJ-MLM         & ---\dag & ---\dag & ---\dag \\
MLM + CPCL + BSJ-MLM  & ---\dag & ---\dag & ---\dag \\
\bottomrule
\end{tabular}
\end{table}
...
Key observations:
\begin{itemize}[leftmargin=*]
  \item \textbf{CPCL is the most effective single pretraining objective.}
        Circular Permutation Contrastive Learning yields the largest gain
        among individual tasks (+1.6 F1, +12.9 AUROC, +2.9 AUPRC over no
        pretraining), validating our hypothesis that learning
        rotation-invariant representations is beneficial for circRNA.
  \item \textbf{MLM provides complementary benefits.}
        Standard MLM improves over no pretraining (+1.3 F1), providing a
        strong sequence context prior.
  \item \textbf{SSP hurts performance.}
        Secondary Structure Prediction ($-0.7$ F1) appears to conflict with
        the fine-tuning objective, possibly because folding-structure labels
        are computed from linearized sequences and do not account for the
        circular backbone.
\end{itemize}
