#!/bin/bash
# MAX — CircMAC no pretrain, seeds 1 2 3
# Usage: ./scripts/max/gpu4_circmac_nopt.sh [GPU_ID]
set -e
GPU=${1:-4}; MODEL="circmac"; MAX_LEN=1022; BS=128; PREFIX="max"
LR=1e-4; EPOCHS=150; EARLYSTOP=20; NUM_WORKERS=4; D_MODEL=128; N_LAYER=6

mkdir -p "logs/${PREFIX}" saved_models
TOTAL=0; SKIPPED=0; RAN=0
echo "=== MAX ${MODEL} no-pretrain (GPU $GPU) ==="
