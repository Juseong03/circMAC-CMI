#!/bin/bash
# Train CircMAC (pretrain=pairing) on ISO-DISJOINT x 3 seeds
# Requires pretrained checkpoint: saved_models/circmac/v2_ptm_pairing/42/pretrain/model.pth
# Usage: bash scripts/final_v2/run_iso_pt_pairing.sh <GPU>
...
echo "=== ISO-DISJOINT CircMAC (pt=pairing) GPU=$GPU seeds=${SEEDS[*]} ==="

for SEED in "${SEEDS[@]}"; do
    EXP="iso_pt_pairing_s${SEED}"
...
echo "=== ISO-DISJOINT CircMAC pt=pairing done ==="
