#!/bin/bash
# Train CircMAC (pretrain=mlm+pairing) on ISO-DISJOINT x seeds
# Requires: saved_models/circmac/v2_ptm_mlm_pairing/42/pretrain/model.pth
# Exp name: iso_pt_mlm_pairing_s{seed}
# Usage: bash scripts/final_v2/run_iso_pt_mlm_pairing.sh <GPU>
...
echo "=== ISO-DISJOINT CircMAC (pt=mlm_pairing) GPU=$GPU seeds=${SEEDS[*]} ==="

for SEED in "${SEEDS[@]}"; do
    EXP="iso_pt_mlm_pairing_s${SEED}"
...
echo "=== ISO-DISJOINT CircMAC pt=mlm_pairing done ==="
