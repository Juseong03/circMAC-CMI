#!/bin/bash
# Train CircMAC (pretrain=mlm_cpcl_ssp) on BSJ-DISJOINT x 3 seeds
# Requires pretrained checkpoint: saved_models/circmac/v2_ptm_mlm_cpcl_ssp/42/pretrain/model.pth
# Usage: bash scripts/final_v2/run_bsj_pt_mlm_cpcl_ssp.sh <GPU>
...
echo "=== BSJ-DISJOINT CircMAC (pt=mlm_cpcl_ssp) GPU=$GPU seeds=${SEEDS[*]} ==="

for SEED in "${SEEDS[@]}"; do
    EXP="bsj_pt_mlm_cpcl_ssp_s${SEED}"
...
echo "=== BSJ-DISJOINT CircMAC pt=mlm_cpcl_ssp done ==="
