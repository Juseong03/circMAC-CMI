#!/bin/bash
# Train CircMAC (pretrain=mlm+ntp+ssp) on BSJ-DISJOINT x seeds
# Requires: saved_models/circmac/v2_ptm_mlm_ntp_ssp/42/pretrain/model.pth
# Exp name: bsj_pt_mlm_ntp_ssp_s{seed}
# Usage: bash scripts/final_v2/run_bsj_pt_mlm_ntp_ssp.sh <GPU>
...
echo "=== BSJ-DISJOINT CircMAC (pt=mlm_ntp_ssp) GPU=$GPU seeds=${SEEDS[*]} ==="

for SEED in "${SEEDS[@]}"; do
    EXP="bsj_pt_mlm_ntp_ssp_s${SEED}"
...
echo "=== BSJ-DISJOINT CircMAC pt=mlm_ntp_ssp done ==="
