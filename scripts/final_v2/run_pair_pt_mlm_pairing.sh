#!/bin/bash
# Train CircMAC (pretrain=mlm+pairing) on PAIR split x seeds
# Requires: saved_models/circmac/v2_ptm_mlm_pairing/42/pretrain/model.pth
# Exp name: v2_pt_mlm_pairing_s{seed}
# Usage: bash scripts/final_v2/run_pair_pt_mlm_pairing.sh <GPU>
...
echo "=== PAIR CircMAC (pt=mlm_pairing) GPU=$GPU seeds=${SEEDS[*]} ==="
...
echo "=== PAIR CircMAC pt=mlm_pairing done ==="
