#!/bin/bash
# sub511 (max_len=511) — iso + bsj, seed=1
# RNA-LMs: batch_size=32, CircMAC: batch_size=64
# Usage: bash scripts/final_v2/bs64_s1/run_sub511.sh <GPU>
...
for SPLIT in iso bsj; do
    ...
    PREFIX="sub511_${SPLIT}"

    # RNAErnie (native max=511, reuse full-length exp if exists)
    ...
    if find saved_models/rnaernie/${EXP} -name "model.pth" 2>/dev/null | grep -q .; then
        ...
        if find saved_models/rnaernie/${EXP} -name "model.pth" 2>/dev/null | grep -q .; then
            ...
            python training.py --model_name rnaernie --device $GPU --task sites \
                --seed $SEED --d_model 128 --n_layer 6 --max_len 511 \
    ...
    # CircMAC (max_len=511, batch_size=64)
