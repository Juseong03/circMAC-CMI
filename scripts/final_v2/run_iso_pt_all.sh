#!/bin/bash
# Train CircMAC (pretrain=all: MLM+NTP+SSP+Pairing) on ISO-DISJOINT x 3 seeds
# Requires pretrained checkpoint: saved_models/circmac/v2_ptm_all/42/pretrain/model.pth
# Usage: bash scripts/final_v2/run_iso_pt_all.sh <GPU>

GPU=${1:-0}
TRAIN_FILE="./data/df_train_iso_disjoint.pkl"
TEST_FILE="./data/df_test_iso_disjoint.pkl"
PT_PATH="saved_models/circmac/v2_ptm_all/42/pretrain/model.pth"
...
if [ ! -f "$PT_PATH" ]; then
    ...
    echo "        Run pretraining first (v2_ptm_all)"
    exit 1
fi

echo "=== ISO-DISJOINT CircMAC (pt=all) GPU=$GPU seeds=${SEEDS[*]} ==="

for SEED in "${SEEDS[@]}"; do
    EXP="iso_pt_all_s${SEED}"
...
echo "=== ISO-DISJOINT CircMAC pt=all done ==="
