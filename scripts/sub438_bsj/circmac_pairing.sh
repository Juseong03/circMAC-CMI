#!/bin/bash
# SUB438 BSJ-DISJOINT — CircMAC + Pairing pretrained, seeds 1 2 3
# Reuses pretrained weights from full-length pretraining (v2_ptm_pairing/42)
# Usage: bash scripts/sub438_bsj/circmac_pairing.sh [GPU_ID]
set -e
GPU=${1:-4}
MODEL="circmac"; MAX_LEN=438; BS=128; PREFIX="sub438_bsj"
...
echo "=== SUB438-BSJ ${MODEL} pairing-pretrained (GPU=$GPU) ==="
