#!/bin/bash
# SUB511 — CircMAC (no pretrain), seeds 1 2 3
# Usage: ./scripts/sub511/gpu4_circmac.sh [GPU_ID]
set -e
GPU=${1:-4}; MODEL="circmac"; MAX_LEN=511; BS=128; PREFIX="sub511"
LR=1e-4; EPOCHS=150; EARLYSTOP=20; NUM_WORKERS=4; D_MODEL=128; N_LAYER=6

mkdir -p "logs/${PREFIX}" saved_models
TOTAL=0; SKIPPED=0; RAN=0
echo "=== SUB511 ${MODEL} (GPU $GPU) ==="

for SEED in 1 2 3; do
    EXP="${PREFIX}_${MODEL}_s${SEED}"
    TOTAL=$((TOTAL+1))
