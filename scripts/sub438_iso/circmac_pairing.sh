#!/bin/bash
# SUB438 ISO-DISJOINT — CircMAC + Pairing pretrained, seeds 1 2 3
# Reuses pretrained weights from full-length pretraining (v2_ptm_pairing/42)
# Usage: bash scripts/sub438_iso/circmac_pairing.sh [GPU_ID]
set -e
GPU=${1:-4}
MODEL="circmac"; MAX_LEN=438; BS=128; PREFIX="sub438_iso"
...
echo "=== SUB438-ISO ${MODEL} pairing-pretrained (GPU=$GPU) ==="
