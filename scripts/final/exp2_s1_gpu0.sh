#!/bin/bash
#===============================================================================
# Exp2: Pretraining Strategy — Server1 GPU0
# Configs: No-PT baseline (3 ft) + MLM (1 pt + 3 ft) = 7 runs
# Usage:  ./scripts/final/exp2_s1_gpu0.sh [GPU_ID]
#===============================================================================
set -e
...
PT_BS=128; PT_LR=5e-4; PT_WD=0.01; PT_EP=200; PT_ES=30
...
echo "=== Exp2 Server1-GPU0: No-PT Baseline + MLM ==="

# ── No-PT Baseline (3 runs) ──────────────────────────────────────────────────
echo "--- Baseline (No Pretraining) ---"
...
if [ ! -f "$PT_PATH" ]; then echo "[SKIP] MLM pretrain model not found"; exit 1; fi
for SEED in "${SEEDS[@]}"; do
    TOTAL=$((TOTAL + 1))
    EXP_NAME="${PREFIX}_mlm_${TASK}_s${SEED}"
    if find "saved_models/circmac/${EXP_NAME}" -name "training.json" 2>/dev/null | grep -q .; then
        echo "[DONE] $EXP_NAME"; SKIPPED=$((SKIPPED + 1)); continue; fi
    RAN=$((RAN + 1))
    echo "[RUN]  $EXP_NAME"
...
echo ""
echo "=== Server1-GPU0 Complete: $RAN ran, $SKIPPED skipped / $TOTAL total ==="
