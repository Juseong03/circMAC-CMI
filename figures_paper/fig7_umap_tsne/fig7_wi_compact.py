...
# Combined figure
# =============================================================================
def make_combined_figure(group_key, red_key, wspace=0.0, show_border=True, stem_suffix=""):
    ...
    stem = f"fig7_wi_{red_key}_{group_key}_minimal_tight_case_aligned{stem_suffix}"
            ...
            for wspace, show_border, suffix in STYLES:
                if SAVE_COMBINED_FIGS:
                    make_combined_figure(
                        group_key, red_key,
                        wspace=wspace, show_border=show_border, stem_suffix=suffix,
                    )
                if SAVE_CASE_FIGS:
...
# =============================================================================
def make_label_only_figure(group_key, red_key, wspace=0.04, show_border=False, stem_suffix=""):
    models = GROUPS[group_key]
    n_models = len(models)
    n_cases = len(CASES)
    ...
    for case_idx, (case_title, case_key) in enumerate(CASES):
        ...
        # Row label: short case name only
...
if __name__ == "__main__":
    main()
    # Also generate label-only versions
    for gk in ("encoder", "pretrained"):
        for rk in ("umap", "tsne"):
            make_label_only_figure(gk, rk, wspace=0.04, show_border=False, stem_suffix="_noborder_gap")