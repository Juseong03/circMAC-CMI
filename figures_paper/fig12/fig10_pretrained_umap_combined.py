#!/usr/bin/env python3
"""
fig10_pretrained_umap_combined.py

Layout per case:
  Rows: (wo_label, wi_label, spacer) × n_cases
  Cols: 5 pretrained models (4 RNA-LM + circMAC)
  N/A placeholder for sequences exceeding positional embedding limits

Output:
  fig10_umap_pretrained_frozen_main.{pdf,png}
  fig10_umap_pretrained_frozen_supp.{pdf,png}
  fig10_umap_pretrained_ft_main.{pdf,png}
  fig10_umap_pretrained_ft_supp.{pdf,png}
"""
...
def main():
    make_figure(MAIN_CASES, MODELS_FROZEN,
                "Pretrained RNA-LM (frozen backbone)  |  Main cases",
                "fig10_umap_pretrained_frozen_main")
    make_figure(SUPP_CASES, MODELS_FROZEN,
                "Pretrained RNA-LM (frozen backbone)  |  Supplementary cases",
                "fig10_umap_pretrained_frozen_supp")
    make_figure(MAIN_CASES, MODELS_FT,
                "Pretrained RNA-LM (fine-tuned backbone)  |  Main cases",
                "fig10_umap_pretrained_ft_main")
    make_figure(SUPP_CASES, MODELS_FT,
                "Pretrained RNA-LM (fine-tuned backbone)  |  Supplementary cases",
                "fig10_umap_pretrained_ft_supp")
