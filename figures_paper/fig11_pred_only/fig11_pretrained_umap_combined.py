#!/usr/bin/env python3
"""
Standalone script: fig11_pretrained_umap_combined

Layout:
  Rows: (wi_pred, spacer) × 3 cases
  Cols: 5 pretrained models (+ circMAC)
  Sil score shown below each prediction panel (outside)
  Case labels on the left, centered on the pred row
"""
...
def add_case_label(fig, axes, pred_row, case_title):
    axes[pred_row, 0].set_ylabel(
        "Prediction\n(w/ miRNA)",
        fontsize=8.0, fontweight="bold",
        rotation=0, ha="right", va="center", labelpad=6,
    )
    pos    = axes[pred_row, 0].get_position()
    y_mid  = 0.5 * (pos.y0 + pos.y1) + CASE_LABEL_Y_OFFSET
    x_case = pos.x0 + CASE_LABEL_X_OFFSET
...
def main():
    ...
    # Row layout: (wi_pred, spacer) × n_cases
    total_rows    = n_cases * 2
    height_ratios = [1.0, 0.28] * n_cases
    height_ratios[-1] = 0.18

    case_row_sets = [(i*2, i*2+1) for i in range(n_cases)]

    fig, axes = plt.subplots(
        total_rows, n_models,
        figsize=(COL_WIDTH * n_models, 8.2),
        gridspec_kw={"height_ratios": height_ratios, "hspace": 0.0, "wspace": WSPACE},
        squeeze=False,
    )

    for _, spacer_row in case_row_sets:
    ...
    for case_idx, (case_title, case_key) in enumerate(CASES):
        pred_row, spacer_row = case_row_sets[case_idx]
        ...
        for ci, (cache_name, display_name, pred_col) in enumerate(MODELS):
            ...
            if case_idx == 0:
                ax_pred.set_title(display_name, fontsize=11, fontweight="bold", pad=8)

    # ── Colorbar ──────────────────────────────────────────────────────────────
    if mappable_store[0] is not None:
        cax = fig.add_axes([0.25, 0.04, 0.68, 0.025])
    ...
    # ── Title ─────────────────────────────────────────────────────────────────
    fig.suptitle("UMAP  |  Pretrained RNA language models  |  w/ miRNA",
                 fontsize=14, fontweight="bold", y=0.992)
    fig.subplots_adjust(left=0.24, right=0.995, top=0.92, bottom=0.13)

    # ── Case labels ───────────────────────────────────────────────────────────
    fig.canvas.draw()
    for case_idx, (case_title, _) in enumerate(CASES):
        pred_row, _ = case_row_sets[case_idx]
        add_case_label(fig, axes, pred_row, case_title)

    stem = "fig11_pretrained_umap_combined"
    for ext in ("pdf", "png"):
