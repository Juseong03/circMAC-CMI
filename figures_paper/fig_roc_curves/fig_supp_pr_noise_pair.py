...
=========================
Supplementary Figure (pair split only): 2-panel
  (A) Precision–Recall curve — CircMAC-BPP + random baseline
  (B) AUPRC under simulated missing-positive annotation rates

Output:
    figures_paper/fig_roc_curves/fig_supp_pr_noise_pair.png
    figures_paper/fig_roc_curves/fig_supp_pr_noise_pair.eps
...
def plot_pr(ax, all_labels, all_probs):
...
def plot_noise(ax, all_labels, all_probs):
    ...
    pd.DataFrame(rows).to_csv(OUT / "fig_supp_pr_noise_pair.csv", index=False)


# ── Main ──────────────────────────────────────────────────────────────────────

def main():
    print(f"Loading predictions: {EXP_NAME}  seeds={SEEDS}")
    ...
    plot_pr(axes[0], all_labels, all_probs)
    plot_noise(axes[1], all_labels, all_probs)

    fig.suptitle(
        "Supplementary Figure: CircMAC-BPP on Pair-Disjoint Split",
        fontsize=11, fontweight="bold", y=1.02,
    )
    fig.tight_layout()

    for ext in ["png", "eps"]:
        p = OUT / f"fig_supp_pr_noise_pair.{ext}"
